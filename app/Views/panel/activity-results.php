...
<div class="property-table-wrap"><table class="operations-table activity-table">
  ...
  <?php foreach ($activity['items'] as $item): ?>
    <tr>
      ...
      <td class="operation-detail"><?php if ($item['last_error']): ?><span class="activity-error"><?= $escape($item['last_error']) ?></span><?php else: ?><?= $item['sync_status'] === 'processing' ? 'Enviando o verificando respuesta.' : ($history ? 'Operacion confirmada.' : 'Pendiente del siguiente ciclo.') ?><?php endif; ?><?php if ($history): ?><small><?= (int) $item['http_status'] > 0 ? 'HTTP ' . (int) $item['http_status'] : 'Validacion local' ?></small><?php endif; ?></td>

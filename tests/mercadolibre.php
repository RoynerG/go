...
check($payload['location']['address_line'] === 'Bocagrande, Cartagena', 'Direccion exacta no se publica en address_line');
check($payload['title'] === '[TEST-1] Apartamento de prueba', 'Titulo lleva el codigo entre corchetes');
check(Builder::effectiveTitle(array_replace($property, ['titulo' => 'TEST-1 ya esta aqui'])) === 'TEST-1 ya esta aqui', 'No duplica el codigo si ya esta en el titulo');
$_ENV['MERCADOLIBRE_SHOW_EXACT_ADDRESS'] = 'true';
check($builder->build($property, 'silver')['location']['address_line'] === 'Direccion de prueba', 'Direccion exacta se publica cuando el env lo permite');
$_ENV['MERCADOLIBRE_SHOW_EXACT_ADDRESS'] = 'false';
$hidePayload = $builder->build($property, 'silver');
$publicHash = Builder::fingerprint($property, 'auto');
$exactHash = Builder::fingerprint($property, 'auto');
$_ENV['MERCADOLIBRE_SHOW_EXACT_ADDRESS'] = 'true';
$exactHash = Builder::fingerprint($property, 'auto');
$_ENV['MERCADOLIBRE_SHOW_EXACT_ADDRESS'] = 'false';
check($publicHash !== $exactHash, 'Cambiar MERCADOLIBRE_SHOW_EXACT_ADDRESS reencola los anuncios');
